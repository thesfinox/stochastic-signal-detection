...
from ssd.utils.utils import (collect_sqlite, collect_values, energy_scale,
                             get_configuration, get_initial_expression,
                             get_output_directory, log_expr, mp_distribution,
                             plot_results, simulation, simulation_distribution)
...
def main(a: argparse.Namespace) -> int | str:
    ...
    # Define the MP distribution
    matrix = cfg.INPUT.MATRIX
    ratio, mp = mp_distribution(cfg, log, output_dir, matrix)

    # Define the energy scale
    _, (m2_bot, m2_top), e_scale, _ = energy_scale(cfg, log, mp)

    # Define the distribution of the simulation
    (m2_bot, m2_top), dist = simulation_distribution(cfg, log, output_dir, ratio, e_scale, m2_bot, m2_top)
